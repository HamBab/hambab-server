...
public interface RestaurantRepository extends JpaRepository<Restaurant, Long> {
    ...
    Optional<List<Restaurant>> findByCategory(@Param("category") String category);

    @Query(value = "select r from Restaurant r where r.name like concat('%',:name,'%')")
    Optional<List<Restaurant>> search(@Param("name") String name);

    @Query(value = "select r from Restaurant r where r.id = :id")
    Optional<Restaurant> getRestaurantInfo(@Param("id") Long id);
}
